...
auto initialize_vector(T value, P dim) {
    return std::vector<T>(dim, value);
}

template<typename T, typename P, typename... Args>
auto initialize_vector(T value, P dim, Args... args) {
    auto out = initialize_vector<T>(value, std::forward<Args>(args)...);
    return std::vector<decltype(out)>(dim, out);
}
